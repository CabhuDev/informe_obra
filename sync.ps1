...
if (-not $DryRun) {
    ...
    try {
        ...
        Write-Host "  ✅ https://obratec.app/form-report - $($response3.StatusCode)" -ForegroundColor Green
    } catch {
        Write-Host "  ❌ https://obratec.app/form-report - Error" -ForegroundColor Red
    }
    
    try {
        $response4 = Invoke-WebRequest -Uri "https://obratec.app/waitlist" -Method HEAD -TimeoutSec 10
        Write-Host "  ✅ https://obratec.app/waitlist - $($response4.StatusCode)" -ForegroundColor Green
    } catch {
        Write-Host "  ❌ https://obratec.app/waitlist - Error" -ForegroundColor Red
    }
}
...
Write-Host "   • https://obratec.app/form-report (ruta beta)" -ForegroundColor White
Write-Host "   • https://obratec.app/waitlist (ruta beta)" -ForegroundColor White
